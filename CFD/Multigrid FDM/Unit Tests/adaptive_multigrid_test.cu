...
void calculateVortInterior(const datatype* stream, datatype* vort_out, const datatype* vort_in, const int n, const datatype dt){

	//Start with the internal points
	for(int x=1; x < n-1; x++){
		for(int y=1; y < n-1; y++){

			vort[x*n + y] = -(stream[x*n + y+1]-stream[x*n + y-1])*
				(vort[(x+1)*n + y]-vort[(x-1)*n + y])/(2.0f*2.0f * dt * dt)+
				(stream[(x+1)*n + y]-stream[(x-1)*n + y])*
				(vort[x*n + y+1]-vort[x*n + y-1])/(2.0f*2.0f * dt * dt)+
				1.0f/(Re * dt * dt) * (-4.0f*vort[x*n + y]+vort[x*n + y+1]+vort[x*n + y-1]+
				vort[(x+1)*n + y]+vort[(x-1)*n + y]);
		}
	}
}
